...
#define TcpIsPacketMarkAsDataStream(head)       (PACKET_FLAG_DATA == (head)->wFlagHead)
            ...
            struct protocol 
            {
                ...
                static nsp_status_t STDCALL parser(void *dat, int cb/*传入数据长度*/, int *pkt_cb/*返回包中记录的不含包头的长度, 也就是build阶段的指定长度*/) 
                {
                    if (!dat) return NSP_STATUS_FATAL;
                ...
                static nsp_status_t STDCALL builder(void *dat, int cb) 
                {
...
#define IEEE104_OPCODE    (0x68)
            ...
            struct protocol 
            {
                ...
                static nsp_status_t STDCALL parser(void *dat, int cb/*传入数据长度*/, int *pkt_cb/*返回包中记录的不含包头的长度, 也就是build阶段的指定长度*/) 
                {
                    ...
                    if (head->op_ != IEEE104_OPCODE) return NSP_STATUS_FATAL;
                ...
                static nsp_status_t STDCALL builder(void *dat, int cb) 
                {
                    if (cb > MAXIMUM_IEEE104_PKTCB) return NSP_STATUS_FATAL;
        ...
        namespace nspdef 
        {
            ...
            struct protocol 
            {
                ...
                static nsp_status_t STDCALL parser(void *dat, int cb, int *pkt_cb) 
                {
                    if (!dat) {
                        return NSP_STATUS_FATAL;
                    }
